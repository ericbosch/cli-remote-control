#!/usr/bin/env bash
#
# Disable Tailscale Serve exposure configured by live_expose_tailscale.sh.
set -euo pipefail
...
if tailscale serve reset 2>/dev/null; then
  :
elif command -v sudo >/dev/null 2>&1 && sudo -n tailscale serve reset; then
  :
else
  log "failed to reset tailscale serve (may need interactive sudo). Run in a terminal:"
