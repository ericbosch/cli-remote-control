#!/usr/bin/env bash
#
# Canonical diagnostics bundle (v8).
...
write_summary_and_reports() {
  ...
  local engine_selector_source="SKIP"
  local api_engines_http="SKIP"
  if [[ -f "${CMD_DIR}/api_engines.http" ]]; then
    api_engines_http="$(cat "${CMD_DIR}/api_engines.http" 2>/dev/null || echo "SKIP")"
    if [[ "${api_engines_http}" == "200" ]]; then
      engine_selector_source="PASS"
    else
      engine_selector_source="FAIL"
    fi
  fi

  local ws_local_auth_header="SKIP"
  local ws_local_ticket="SKIP"
  local ws_serve_auth_header="SKIP"
  local ws_serve_ticket="SKIP"
  local ws_tailnet_direct="SKIP"
  if [[ -f "${CMD_DIR}/ws_matrix_check.txt" ]]; then
    ws_local_auth_header="$(rg -n '^local_ws_auth_header=' "${CMD_DIR}/ws_matrix_check.txt" | head -n 1 | cut -d= -f2- | awk '{print $1}' | tr -d '\r' || echo "SKIP")"
    ws_local_ticket="$(rg -n '^local_ws_ticket=' "${CMD_DIR}/ws_matrix_check.txt" | head -n 1 | cut -d= -f2- | awk '{print $1}' | tr -d '\r' || echo "SKIP")"
    ws_serve_auth_header="$(rg -n '^serve_wss_auth_header=' "${CMD_DIR}/ws_matrix_check.txt" | head -n 1 | cut -d= -f2- | awk '{print $1}' | tr -d '\r' || echo "SKIP")"
    ws_serve_ticket="$(rg -n '^serve_wss_ticket=' "${CMD_DIR}/ws_matrix_check.txt" | head -n 1 | cut -d= -f2- | awk '{print $1}' | tr -d '\r' || echo "SKIP")"
    ws_tailnet_direct="$(rg -n '^tailnet_ws_direct=' "${CMD_DIR}/ws_matrix_check.txt" | head -n 1 | cut -d= -f2- | awk '{print $1}' | tr -d '\r' || echo "SKIP")"
    if [[ -z "${ws_local_auth_header}" ]]; then ws_local_auth_header="SKIP"; fi
    if [[ -z "${ws_local_ticket}" ]]; then ws_local_ticket="SKIP"; fi
    if [[ -z "${ws_serve_auth_header}" ]]; then ws_serve_auth_header="SKIP"; fi
    if [[ -z "${ws_serve_ticket}" ]]; then ws_serve_ticket="SKIP"; fi
    if [[ -z "${ws_tailnet_direct}" ]]; then ws_tailnet_direct="SKIP"; fi
  fi
  ...
  if [[ "${systemd_present}" == "PASS" ]]; then
    user_load_state="$(systemctl --user show "${SERVICE_NAME}" -p LoadState --value 2>/dev/null || true)"
    if [[ "${user_load_state}" == "loaded" ]]; then
      ...
      systemd_user_service_active="SKIP"
      systemd_user_service_enabled="SKIP"
    fi

    system_load_state="$(systemctl show "${SERVICE_NAME}" -p LoadState --value 2>/dev/null || true)"
    if [[ "${system_load_state}" == "loaded" ]]; then
      ...
      systemd_system_service_active="SKIP"
      systemd_system_service_enabled="SKIP"
    fi
  fi
  ...
  if [[ "${host_listen}" == "PASS" && "${unauth_ok}" == "PASS" && "${auth_ok}" == "PASS" && "${healthz_ok}" == "PASS" && "${ws_ticket_ok}" == "PASS" && "${engine_selector_source}" == "PASS" && "${redaction_status}" == "PASS" && "${git_worktree_clean}" == "PASS" ]]; then
    if [[ "${e2e_gate_ok}" != "PASS" ]]; then
      go="NO-GO"
    elif [[ "${web_console_error_signature}" == "FAIL" ]]; then
      go="NO-GO"
    elif [[ "${ws_serve_ticket}" != "SKIP" && "${ws_serve_ticket}" != "PASS" ]]; then
      go="NO-GO"
...

  {
    ...
    printf 'engine_selector_source=%s (http=%s)\n' "${engine_selector_source}" "${api_engines_http}"
    ...
    printf 'ws_matrix_local_auth_header=%s\n' "${ws_local_auth_header}"
    printf 'ws_matrix_local_ticket=%s\n' "${ws_local_ticket}"
    printf 'ws_matrix_serve_auth_header=%s\n' "${ws_serve_auth_header}"
    printf 'ws_matrix_serve_ticket=%s\n' "${ws_serve_ticket}"
    printf 'ws_matrix_tailnet_direct=%s\n' "${ws_tailnet_direct}"
  ...
  # REPORT.md: human narrative w/ explicit detected capabilities
  {
    ...
    echo "- engine_selector_source: ${engine_selector_source} (http=${api_engines_http})"
    ...
    echo "- ws_matrix: local(auth=${ws_local_auth_header}, ticket=${ws_local_ticket}) serve(auth=${ws_serve_auth_header}, ticket=${ws_serve_ticket}) tailnet_direct=${ws_tailnet_direct}"
    ...
    echo "- WS transport matrix: cmd/ws_matrix_check.txt (no secrets)"
  ...
  {
    echo "# cli-remote-control — Staff summary (v8)"
    echo
    echo "- generated_at: ${TS}"
    echo "- diag_dir: ${OUT_DIR}/"
    echo "- diag_zip: ${ZIP_PATH}"
    ...
    echo "- e2e_input_roundtrip=${e2e_input_roundtrip}"
    echo "- e2e_output_marker=${e2e_output_marker}"
    echo "- ws_matrix_local_auth_header=${ws_local_auth_header}"
    echo "- ws_matrix_local_ticket=${ws_local_ticket}"
    echo "- ws_matrix_serve_auth_header=${ws_serve_auth_header}"
    echo "- ws_matrix_serve_ticket=${ws_serve_ticket}"
    echo "- ws_matrix_tailnet_direct=${ws_tailnet_direct}"
    echo "- cursor_engine_entrypoint=${cursor_engine_entrypoint}"
    echo "- engines: codex=${codex_present}, cursor_cmd=${cursor_cmd_present}, cursor_ide=${cursor_ide_installed}, agent=${agent_bin_present}, cursor-agent=${cursor_agent_bin_present}"
    echo
    echo "## Notes"
...
main() {
  ...
  auth_request_keys_only "GET" "${BASE_URL}/api/engines" "${CMD_DIR}/api_engines.http" "${CMD_DIR}/api_engines.keys"
  ...
  # Capture /api/engines body (safe: engine names only; no secrets expected).
  if [[ -f "${TOKEN_FILE}" ]]; then
    capture_shell "api_engines_body.txt" "token=\"\$(tr -d '\\r\\n' < \"${TOKEN_FILE}\")\"; curl -sS -K - <<EOF\nurl = \"${BASE_URL}/api/engines\"\nheader = \"Authorization: Bearer \${token}\"\nEOF"
  else
    echo "SKIP (token file missing)" > "${CMD_DIR}/api_engines_body.txt"
  fi
  ...
  # WS transport matrix (local ws + Serve wss best-effort; never prints token/ticket)
  if [[ -x "${ROOT}/scripts/ws_matrix_check.sh" ]]; then
    capture_shell "ws_matrix_check.txt" "RC_HOST_PORT='${PORT}' RC_LOCAL_BASE='${BASE_URL}' RC_TOKEN_FILE='${TOKEN_FILE}' ${ROOT}/scripts/ws_matrix_check.sh"
  else
    echo "ws_matrix=SKIP (missing scripts/ws_matrix_check.sh)" > "${CMD_DIR}/ws_matrix_check.txt"
  fi
